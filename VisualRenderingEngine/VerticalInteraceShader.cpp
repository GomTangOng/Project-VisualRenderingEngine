...
CVerticalInteraceShader::CVerticalInteraceShader()
{
}
...
void CVerticalInteraceShader::Render()
{
	OnPreRender();
...
void CVerticalInteraceShader::OnPreRender()
{
	CShader::OnPreRender();
	ID3D11ShaderResourceView *pSRVNull[1]{ nullptr };
...
void CVerticalInteraceShader::BuildViews(const UINT width, const UINT height)
{
	Memory::Release(m_pOutSRV);
	Memory::Release(m_pOutUAV);
	Memory::Release(m_pOutRTV);
...
void CVerticalInteraceShader::UpdateConstantBuffer()
{
