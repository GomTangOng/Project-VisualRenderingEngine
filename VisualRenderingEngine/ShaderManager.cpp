...
HRESULT CShaderManager::CreateComputeShaderFromFile(WCHAR * pFileName, LPCSTR pEntryPoint, LPCSTR pShaderModel, ID3D11ComputeShader ** ppd3dComputeShader)
{
	HRESULT hr = S_OK;
	...
	dwShaderFlags |= D3DCOMPILE_SKIP_OPTIMIZATION;
#endif
	ID3DBlob *pComputeShaderBlob = nullptr, *pErrorBlob = nullptr;
	hr = D3DCompileFromFile(pFileName, nullptr, nullptr, pEntryPoint, pShaderModel,
		dwShaderFlags, 0, &pComputeShaderBlob, &pErrorBlob);
	if (FAILED(hr))
	{
		if (pErrorBlob)
		{
	...
	{
		m_pDevice->CreateComputeShader(pComputeShaderBlob->GetBufferPointer(), pComputeShaderBlob->GetBufferSize(), nullptr, ppd3dComputeShader);
